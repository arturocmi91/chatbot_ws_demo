...
function iso2text(iso) {
    try {
        //Convertir la fecha a DataTime de luxon.
        const dateTime = DateTime.fromISO(iso, { zone: 'utc' }).setZone('Europe/Madrid');

        // Formatear la fecha.
        const fomattedDate = dateTime.toLocaleString({
            weekday: 'long',
            day: '2-digit',
            month: 'long',
            hour: '2-digit',
...
async function text2iso(text) {
    ...
    const prompt = "La fecha de hoy es: " + currentDate + `Te voy a dar un texto.
        ...
        Si el texto es algo como: "Mañana 10am", sumarle un día a la fecha actual y dar eso como 
        resultado.
